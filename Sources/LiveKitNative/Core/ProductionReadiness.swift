...
public extension LiveKitNative {
    static let productionReadiness = LiveKitNativeProductionReadiness(
        ...
        blockers: [
            "ICEAgent nomination, local ICE trickle signaling, server ICE server configuration application, STUN UDP server-reflexive candidate gathering, bound local ICE UDP host candidate gathering and socket reuse for STUN checks and media datagrams, DTLS-SRTP packet protection, SDP DTLS role/fingerprint extraction, handshaker-backed media-session binding with result role/profile validation, injected Room publisher/subscriber media startup, injected publisher transport teardown, a Room publisher RTP bridge through injected secure media transport, and stateful Opus/H.264 publisher RTP packetizer bridges exist, but live peer connections still need a real DTLS handshake/exporter and default subscriber/publisher runtime integration.",
            "Fresh join, resume reconnect, disconnect, and failed signal-loop boundaries regenerate local ICE credentials, clear stale peer negotiation state, parse TURN UDP/TCP/TLS endpoint configuration, exercise TURN Allocate, Refresh, CreatePermission, and ChannelBind request/authentication/response-validation primitives with one-shot stale nonce retry, cover TURN ChannelData framing, provide deterministic allocation/permission maintenance scheduling, and expose a ChannelData relay transport over an abstract media datagram transport in unit tests, but full ICE restart signaling, TURN relay allocation/socket integration, timer-driven TURN network maintenance, UDP/TCP/TLS fallback, and live media recovery are not implemented.",
            ...
            "General production video meetings require jitter buffering, packet-loss recovery, RTCP feedback handling, bandwidth estimation, congestion control, adaptive quality, and bounded backpressure/frame-drop policy.",
