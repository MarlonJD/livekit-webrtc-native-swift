...
public extension LiveKitNative {
    static let productionReadiness = LiveKitNativeProductionReadiness(
        ...
        blockers: [
            "ICEAgent nomination, local ICE trickle signaling, server ICE server configuration application, STUN UDP server-reflexive candidate gathering, bound local ICE UDP host candidate gathering and socket reuse for STUN checks and media datagrams, DTLS-SRTP packet protection, SDP DTLS role/fingerprint extraction, handshaker-backed media-session binding, injected Room publisher/subscriber media startup, and injected publisher transport teardown exist, but live peer connections still need a real DTLS handshake/exporter and default subscriber/publisher runtime integration.",
            "Fresh join, resume reconnect, disconnect, and failed signal-loop boundaries regenerate local ICE credentials, clear stale peer negotiation state, parse TURN UDP/TCP/TLS endpoint configuration, and encode/decode TURN Allocate request primitives in unit tests, but full ICE restart signaling, TURN relay allocation/fallback, and live media recovery are not implemented.",
