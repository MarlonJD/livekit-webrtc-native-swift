...
public extension LiveKitNative {
    static let productionReadiness = LiveKitNativeProductionReadiness(
        ...
        blockers: [
            ...
            "Publisher camera capture can encode H.264 through VideoToolbox with bounded frame backpressure/drop control and publisher microphone capture can encode Opus through AudioToolbox before RTP/SRTP send, while subscriber RTP can pass through jitter buffering, H.264/Opus packet assembly, NACK/PLI feedback, opt-in Opus decode-to-audio-playout scheduling, opt-in H.264 VideoToolbox decode-to-pixel-buffer scheduling, and application-provided subscriber video renderer handoff; platform video display validation, LiveKit E2E media validation, meeting-grade audio session behavior, and production runtime pacing remain incomplete.",
            "Production H.264 now uses real VideoToolbox encode output for publish smoke coverage and opt-in VideoToolbox decode output for subscribe smoke coverage, but production readiness still requires platform display validation, hardware acceleration verification where the OS exposes that signal, and explicit fallback behavior instead of relying on a pure Swift codec implementation.",
